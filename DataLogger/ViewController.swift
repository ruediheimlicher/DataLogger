...
//   @IBOutlet  var dataAbszisse_Volt: Abszisse!
  ...
  // @IBOutlet weak var deviceCollectionView: NSCollectionView!
   ...
   func checkDeviceStatus(callback_status:UInt8) -> Bool
   {
      ...
      for devicelinie in swiftArray
      {
         ...
         if (callback_status & (1<<devicecode) > 0)
         {
...
//      print("antwort: \(antwort)")
      ...
      let chars: [UInt8] =  [21, 27, 24, 94, 233, 211, 222, 233, 244, 255, 55, 255, 255, 0, 01, 20]
      let count = chars.count / MemoryLayout<UInt8>.size
      let c = MemoryLayout<Int>.size
...
/*      
      let REFS1:UInt8 = 7
      let REFS0:UInt8 = 6
      var ADMUX:UInt8 = 0x06
      let derKanal:UInt8 = 0
 ...
 */     
      ...
      nc.addObserver(forName:Notification.Name(rawValue:"newdownload"),// Name im Aufruf in usb.swift
         object:nil, queue:nil,
         using:newDownloadAktion)
...
//      tempDic["device"] = "abcd" //devicearray[0]
 //     tempDic["deviceID"] = "0"
      ...
      tempDic["A0"] = String(0)
      tempDic["analogAtitel"] = "ADC 2\tADC 3\tAAA 4\tADC"
      tempDic["A1"] = String(1)
...
//      tempDic["device"] = devicearray[1]
 //     tempDic["deviceID"] = "1"
      ...
      tempDic["A1"] = String(1)
      tempDic["A"] = String(7)
      tempDic["analogAtitel"] = "LM35\tKTY\tPT1000\t--"
...
//MARK: READ_START      
      case READ_START:
         ...
         let batt = UInt16((teensy.read_byteArray[USB_BATT_BYTE])) 
 ...
 //        print(" teensy0: \(teensy0) ")
      ...
      case CHECK_WL:
         ...
         {
            let startdevicebatteriespannung = Float(teensy.read_byteArray[DATA_START_BYTE + zeilenindex]) * 2
            print("CHECK_WL zeilenindex: \(zeilenindex) startdevicebatteriespannung: \(startdevicebatteriespannung)")
            if startdevicebatteriespannung > 0
            {
               let s = String(format:"%2.02fV", startdevicebatteriespannung/100)
               swiftArray[zeilenindex]["batterie"] = String(format:"%2.02fV", startdevicebatteriespannung/100)
               print("check_WL startdevicebatteriespannung: \(startdevicebatteriespannung)")
            }
         }
...
*/
            ...
            //print("Header: ")
            download_sd_progress.intValue = 0 // Feld zuruecksetzen
            ...
            print("LOGGER_START lastpacket: \(lastpacket)")
           ...
           // lastpacket = 0xFF
            ...
            IntervallPop.selectItem(withObjectValue: download_intervall)
      ...
      case LOGGER_CONT: // weiteres Paket (Datenzeile)
        // print("newLoggerDataAktion LOGGER_CONT: \(code) packetcount: \(teensy.read_byteArray[PACKETCOUNT_BYTE])")
         //print("newLoggerDataAktion LOGGER_CONT  \nraw data:\n\(teensy.read_byteArray)\n")
          ...
          packetcount = UInt8(teensy.read_byteArray[PACKETCOUNT_BYTE]) // Byte 8
         ...
         {
            print("lastpacket fertig geladen bei packetcount: \(packetcount)")
            last = true
         }
         ...
         else
         {
           ...
           /*
            ...
            else
            {
               ...
               //print("\(dataname)\n")
               //print("downloadDataFeld: \n\(downloadDataFeld.string!)")
               //print("loggerDataArray: ")
               /*
              for l in 0..<loggerDataArray.count
               {
                  print (loggerDataArray[l])
               }
 */
      ...
      case LOGGER_NEXT: // weiterer Block
         ...
         print ("")
         //print ("controlstring")
         //let controlstring = String(data: Data(teensy.read_byteArray), encoding: .utf8)
         //print(controlstring)
         ...
         if (packetcount < 20) // 480 bytes pro block,, noch weiterfahren
         {            
             cont_log_USB(paketcnt: (packetcount))            
         }
         else
      ...
      case LOGGER_STOP:
         ...
         let intervall = teensy.read_byteArray[TAKT_LO_BYTE] | (teensy.read_byteArray[TAKT_LO_BYTE+1] << 8)
  ...
  //       var senderfolg = teensy.start_write_USB()
      ...
      case MESSUNG_STOP:
         ...
         let counterHI = Int32(teensy.read_byteArray[DATACOUNT_HI_BYTE])
         let counter = (counterLO ) | ((counterHI )<<8)
         print("counter Messung stop: \(counter) ")
         messungcounter.intValue = counter
         
         cont_read_check.state = NSControl.StateValue.off
         ...
         print("messung stop blockposition: blockposition: \(blockposition) blockcounter: \(blockcounter.intValue) blockcounter set")
...
*/         
         ...
         if (DiagrammDataArray.count > 1) // es hat angefangen
         {
            var messungstring:String = MessungDataString(data:DiagrammDataArray) // String aus Array der Daten
      ...
      case MESSUNG_DATA: // wird gesetzt, wenn vom Teensy im Timertakt Daten gesendet werden
         ...
         //print("messungnummer: \(messungnummer)\ndevicenummer: \(devicenummer)\(teensy.read_byteArray)\n")
...
//         print ("devicenummer: \(devicenummer)\tchannelnummer: \(channelnummer)")
   ...
   //      let devicecount = (teensy.read_byteArray[DEVICECOUNT_BYTE])
         ...
         for devicelinie in swiftArray // deviceindex ist loop-counter
         {
            //var zeile = devicelinie
            let device = devicelinie["device"]!
            let analog = devicelinie["A"]! // Tastenstatus Kanaele           
            ...
            {
               print("device \(String(describing: device)) ist nicht mehr da")
               if (oldstatus == 1)
               {
                  //print("neu weg")
                  swiftArray[deviceindex]["on"] = "0"
         ...
         let blockposition = (Int32(teensy.read_byteArray[BLOCKOFFSETLO_BYTE]) & 0x00FF) | ((Int32(teensy.read_byteArray[BLOCKOFFSETHI_BYTE])  & 0x00FF)<<8)
         //print("messung data device: \(devicenummer) blockposition: \(blockposition) blockcounter: \(blockcounter.intValue) blockcounter set")
 ...
 //        var tempinputDataFeldstring = String(tagsekunde()-MessungStartzeit) + "\t" + messungcounter.stringValue + "\t" 
         ...
         {
         case 0:     
            //print("\tMESSUNG_DATA device 0 aktiv masterstatus; \(masterstatus)")
            ...
            let teensybatteriefloat = Float(teensybatterie) * 2 / 100
            teensybatt.stringValue = NSString(format:"%.2f", teensybatteriefloat) as String
            //print("teensybatterie: \(teensybatterie) teensybatteriefloat: \(teensybatteriefloat)")      
            swiftArray[devicenummer]["batterie"] =  String(format:"%2.02fV", teensybatteriefloat)
 ...
 // Part 2
         ...
         case 1:
            ...
            //print("\nMESSUNG_DATA device 1 aktiv")
            //print ("")
  /*
            print("task 1 CODE read_byteArray\tcount: \(counter)\t")
...
*/
    ...
    //        var temparray = teensy.read_byteArray[DATA_START_BYTE...(BUFFER_SIZE-1)] // Teilarray mit Daten, DATA_START_BYTE: 16
            ...
            if (wl_callback_status & (1<<UInt8(devicenummer)) > 0) // device ist aktiv
            {
               //print("\ndevice 1 aktiv")
 ...
 //              print("wl_callback_status: \(wl_callback_status) devicenummer: \(devicenummer)  devicestatus: \(devicestatus)")
    ...
    //           print("device \(String(describing: device)) ist da")
               ...
               let devicebatteriespannung = Float(teensy.read_byteArray[USB_BATT_BYTE])  * 2
              
               
       //        print ("switch task: \(devicenummer)\t devicebatteriespannung: \(devicebatteriespannung/100)")
               
               swiftArray[devicenummer]["batterie"] = String(format:"%2.02fV", devicebatteriespannung/100)

               
            //      print("pt: \(teensy.read_byteArray[15])")
               ...
               messungfloatarray[devicenummer][DIAGRAMMDATA_OFFSET + 0] = analog0float
               ...
               messungfloatarray[devicenummer][DIAGRAMMDATA_OFFSET + 2] = analog2float
               ...
               messungfloatarray[devicenummer][DIAGRAMMDATA_OFFSET + 3] = analog3float
               ...
               let batteriespannung = Int32(teensy.read_byteArray[USB_BATT_BYTE]) * 2
               batteriefloat = Float(batteriespannung)
               
 //              print("device 1 batteriefloat USB_BATT_BYTE: \(batteriefloat)");
               let batt:String? = swiftArray[devicenummer]["batterie"]
               if (batteriefloat < Float(BATT_MIN))
               {
                  ...
                  if let batt = swiftArray[devicenummer]["batterie"]
                  {
 ...
 */   
   ...
   */         
            ...
            if (wl_callback_status & (1<<UInt8(devicenummer)) > 0)
            {
               devicestatus |= (1<<UInt8(devicenummer))
               let device = swiftArray[devicenummer]
               //print("device \(String(describing: device)) ist da")
 ...
 //              print("wl_callback_status: \(wl_callback_status) devicenummer: \(devicenummer)  devicestatus: \(devicestatus)")
   ...
   //            print ("switch task: \(devicenummer)\t devicebatteriespannung: \(devicebatteriespannung/100)")
               ...
               let devicebatteriefloat = Float(devicebatteriespannung)  / 100
               swiftArray[devicenummer]["batterie"] = String(format:"%2.02fV", devicebatteriefloat)
 //              print ("switch task: \(devicenummer)\t devicebatteriespannung: \(devicebatteriefloat)")
               ...
               messungfloatarray[devicenummer][DIAGRAMMDATA_OFFSET + 0] = analog0float  // DIAGRAMMDATA_OFFSET 4
  ...
  //             Vertikalbalken.setLevel(Int32(analog2float*0xFF/2500))
               ...
               if (batteriefloat < Float(BATT_MIN))
               {
                  print("low batt: \(String(describing: swiftArray[devicenummer]["batterie"]))")
               }
               // print ("")
            }
         default:
...
*/         
   ...
   //         inputDataFeld.textStorage?.append(NSAttributedString(string:(tempinputDataFeldstring + "\n")))
            ...
            let tempstring = tempinputDataFeldstring + "\n"
            //         inputDataFeld.string = "Messung tagsekunde: \(zeit)\n"
            
            let paragraphStyle = NSMutableParagraphStyle()
            // print("tabstops A: \(paragraphStyle.tabStops)")
            
            let tabInterval:CGFloat = 48.0
            paragraphStyle.tabStops.removeAll()
...
//         print("end for device.  devicestatus: \(devicestatus) wl_callback_status: \(wl_callback_status) devicestatus: \(devicestatus)")
         ...
         //print("result setWerteArray:0 \(werteArray[0])")
         //if (werteArray[0][0]
         // MARK:*** setWerteArray:
 ...
 */     
 ...
 //     Start_Logger.isEnabled = false
      ...
      teensy.write_byteArray[DATACOUNT_LO_BYTE] = UInt8(messungcounter.intValue & 0x00FF) // byte 3
      teensy.write_byteArray[DATACOUNT_HI_BYTE] = UInt8((messungcounter.intValue & 0xFF00)>>8)
 ...
 */
   ...
   @IBAction func report_start_messung(_ sender: NSButton)
   {
      ...
      if (sender.state.rawValue == 1)
      {
         ...
         wl_callback_status_Feld.intValue = 0
         blockcounter.intValue = 0
         TaskListe.reloadData()
         devicestatus = 0x01
...
//         cont_read_check.state = 0;
   ...
   //      teensy.write_byteArray[ABSCHNITT_BYTE] = 0
         ...
         print("report_start_messung messungcounter intvalue: \(messungcounter.intValue) messungcounter wird 0")
         messungcounter.intValue = 0;
...
//         {            
         ...
         if teensy.readtimervalid() == true
         {
            print("report start Messung readtimer valid vor")
         }
         else 
         {
            print("report start Messung readtimer not valid vor")
            var start_read_USB_erfolg = teensy.start_read_USB(true)
 ...
 */  
   func stop_messung()
   {
      ...
      teensy.write_byteArray[0] = UInt8(MESSUNG_STOP) // C1
 ...
 */
      ...
      if teensy.readtimervalid() == true
      {
         print("check wl readtimer valid vor")
      }
      else 
      {
         print("check wl readtimer not valid vor")
         var start_read_USB_erfolg = teensy.start_read_USB(true)
         print("check wl  erfolg: \(start_read_USB_erfolg)")
      }
 ...
 //     }
   ...
   func reorderAbszissemitPosArray(posarray:[UInt16])
   {
   for ordinate in ordinateArray
   {
      ordinate.isHidden = true
...
*/   
 ...
 //           teensy.write_byteArray[ABSCHNITT_BYTE] = 0
     ...
     //       read_sd_startblock.intValue = Int32(startblock)
            ...
            let zeit = tagsekunde()
            print("report_start_read_USB startblock: \(startblock)  zeit: \(zeit)")
 ...
 */              
            ...
            {
               print("report start Messung readtimer not valid vor")
               let readerr = teensy.start_read_USB(true)
               self.codeFeld.integerValue = readerr
            }
/*
...
*/
      ...
      var zeilenindex = 0
      let headerlines = 2
      var messungnummer:UInt16 = 0 // laufende Nummer der Messungen
      var neuemessung = false
      var NullpunktOffset:Int = 0
      var messungarray = [[UInt16]]() // Array mit daten der gleichen Messungnummer. Ergibt wertearray fuer Messungnummer
      var posarray:[UInt16] = [] // aktive devices, fortlaufend mit devicenummer
      ...
      for datazeile in loggerdataArray
      {
         ...
         */
         ...
         {
            // messungnummer lesen
            let tempmessungnummer:UInt16 = UInt16(zeilenarray[2])! | ((UInt16(zeilenarray[3])!<<8))
            ...
            //print("tempmessungnummer: \(tempmessungnummer) zeilenindex: \(zeilenindex)")
            if (tempmessungnummer > 0) && (tempmessungnummer > messungnummer )// neue Messung beginnt, 
            {
               neuemessung = true
               //print("\nmessungnummer: \(messungnummer) messungarray:")
             //  for zeile in messungarray
              // {
    ...
    //           print ("messungnummer: \(messungnummer)  zeilenindex: \(zeilenindex) anz: \(werteArray.count) array: ")
               for zeile in werteArray
               {
   //               print(zeile)
                  let tempdeviceID = UInt16(zeile[1])
                  
                  if posarray.contains(tempdeviceID) 
                  {
                     //print("device \(tempdeviceID) schon da")
                  }
                  else
                  {
                     posarray.append(tempdeviceID)
                  }
               ...
               if (werteArray.count > 1)
               {
                  
                  datagraph.setWerteArray(werteArray:werteArray,  nullpunktoffset: NullpunktOffset)
                  //self.datagraph.wertesammlungarray.append(werteArray)
               }
               //print("*")
               messungnummer = tempmessungnummer
               if (messungarray.count > 0)
               {
                  //print("tempmessungnummer: \(tempmessungnummer) messungarray: \(messungarray)")
                  messungarray.removeAll()
                  
               }
               messungarray.append(tempIntZeilenArray)
       ...
       //print("posarray: \(posarray)")
      reorderAbszissemitPosArray(posarray: posarray)
      //self.datagraph.printwertesammlung()
      
      //print("\n Datenarray: ")
      //for zeile in self.datagraph.DatenArray
      //{
         //print("\(zeile)")
      //}
      datagraph.display()
   ...
   open func werteArrayFromIntMessungArray(data:[[UInt16]], messungnummer:UInt16) -> [[Float]]
   {
      //print("werteArrayFromIntMessungArray data eingang: messungnummer: \(messungnummer) ")
      var AnzeigeFaktor:Float = 1.0 // Faktor für y-wert, abhängig von Abszisse-Skala
      var SortenFaktor:Float = 1.0 // Anzeige in Diagramm durch Sortenfaktor teilen
      ...
      werteArray.append([Float(messungnummer),1.0,1.0,1.0])// Abszisse
      /*
      for zeile in data
      {
         print("Zeile: \(zeile)")
      ...
      for intarray in data
      {
         ...
         let deviceID = Int(intarray[0]) // devicenummer
         ...
         for kanal in 0..<8
         {
            if (kanalstatus & (1<<kanal) > 0)
            {
               ...
               case 1:
                  ...
                  {
                  case 0: // LM35
                     wert_norm = Float(wert) / 10.0 // LM-Wert kommt mit Faktor 10
                     
                  case 1: // KTY
                     wert_norm = Float(wert) // KTY_FAKTOR
                  case 2: // PT100
                     wert_norm = Float(wert)
                  case 3: // aux
                     wert_norm = Float(wert)
               ...
               case 2:  // ADC12BIT
                  ...
                  {
                     //print("kanal 1,3")
                     wert_norm = Float(wert) / 0x1000 * 4.096 * 40
                     SortenFaktor = 10 
                  }
               //print("wert_norm: \(wert_norm)")
               default: 
               ...
               werteArray.append([wert_norm, id, SortenFaktor, AnzeigeFaktor])
      ...
      print("* end werteArrayFromIntMessungArray")
      */
   return werteArray
   }
   ...
   open func werteArrayFromIntArray(data:[UInt16]) -> [[Float]]
   {
      ...
      //print("werteArrayFromIntArray messungcounter: \(messungcounter)")
       ...
       werteArray[0] = [Float(messungcounter),1.0,1.0,0.0] // Abszisse
...
//      werteArray.append(Float(messungcounter))
  ...
  //       print("kanal: \(kanal) wert: \(wert) wert_norm: \(wert_norm)")
      ...
      if (((lastx) + docviewx ) > grenze) // docviewx ist negativ, wenn gegen links gescrollt wurde
      {
         ...
         print("werteArray lastdata zu gross \(lastx) delta:  \(delta)")
         self.dataScroller.documentView?.frame.origin.x -=   CGFloat(delta)
 ...
 //     print("+++++++++++++++++++++++++++++++++\n");
   ...
   func newDownloadAktion(notification:Notification) -> Void
   {
      // nicht verwendet, nur test fuer Notification
      //print("newDownloadAktion userInfo: ")
  ...
  //          print("\(zeile)")
   ...
   //      }
         ...
         //print("newDownloadAktion data: \(userInfo?["data"])")
 */
      }
   }
...
/*
extension NSTextView : NSTextDelegate
{
